...
document.getElementById('search-btn').addEventListener('click', async () => {
  ...
  availableRooms.data.sort((a, b) => a.name.localeCompare(b.name));
  ...
  availableRooms.data.forEach((room) => {
    ...
    // เพิ่มหัวข้อสิ่งอำนวยความสะดวก
    const facilitiesTitle = document.createElement('p');
    facilitiesTitle.textContent = 'Facilities:';
    facilitiesTitle.style.textAlign = 'left'; // ชิดซ้าย
    roomCard.appendChild(facilitiesTitle);

    // เพิ่มรายการสิ่งอำนวยความสะดวกพร้อมภาพ
    const facilitiesContainer = document.createElement('div'); // ใช้ div เพื่อแสดงรายการสิ่งอำนวยความสะดวก
    facilitiesContainer.style.display = 'flex'; // ใช้ flex เพื่อจัดเรียงภาพและชื่อให้อยู่ในแถวเดียวกัน
    facilitiesContainer.style.flexWrap = 'wrap'; // ถ้าหลายรายการให้ขึ้นบรรทัดใหม่

    room.facilities.forEach(facility => {
      const facilityItem = document.createElement('div'); // ใช้ div สำหรับแต่ละรายการ
      facilityItem.className = 'facility-item'; // เพิ่ม class เพื่อจัดการด้วย CSS

      // เพิ่มข้อความ
      const facilityText = document.createElement('span');
      facilityText.textContent = "-"+facility;

      facilityItem.appendChild(facilityText); // เพิ่มข้อความเข้าไปใน div
      facilitiesContainer.appendChild(facilityItem); // เพิ่ม div เข้า container
    });

    
    roomCard.appendChild(facilitiesContainer);
